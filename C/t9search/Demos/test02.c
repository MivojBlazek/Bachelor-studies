...
/** HLEDANI PARAMETRU -s **/
	if (argc == 3)
	{
		...
		for (int a = 0; (argv[1][a] != '\0') || (s_str[a] != '\0'); a++)
		{
			if (argv[1][a] != s_str[a])
			{
				fprintf(stderr, "Unknown parameter %s! You can use '-s'\n", argv[1]);
				return 1;
			}
		}
		mode = 2;
...
/** CTENI SOUBORU, POROVNAVANI SE SUBSTRINGEM A VYPIS **/
	...
	//
	//
	/** TU BUDE IMO TO HLEDANI S PRERUSENOU POSLOUPNOSTI ZNAKU **/
