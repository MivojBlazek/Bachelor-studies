...
void init_cluster(struct cluster_t *c, int cap)
{
    ...
    //
    c->size = 0;
    c->capacity = cap;/* *(int *)malloc(cap * sizeof(struct obj_t));*/
...
void append_cluster(struct cluster_t *c, struct obj_t obj)
{
    ...
    {
        resize_cluster(c, c->capacity ? c->capacity + c->capacity : 1);
    }
    if (c->size < c->capacity)
    {
        c->obj[c->size] = obj;
...
void merge_clusters(struct cluster_t *c1, struct cluster_t *c2)
{
    ...
    {
        resize_cluster(c1, c1->capacity ? c1->capacity + c1->capacity : 1);
    }
...
int remove_cluster(struct cluster_t *carr, int narr, int idx)
{
    ...
    for (int i = 0; i < narr; i++)
    {
        ...
        {
//  -->  nestacilo by tohle ?
            //carr->obj[idx] = NULL;	//musi to tam byt???
            for (int j = idx; j < narr - 1; j++)
            {
                carr->obj[j] = carr->obj[j + 1];
            }
//  <--  nestacilo by tohle ?
            break;
        }
...
int load_clusters(char *filename, struct cluster_t **arr)
{
    ...
    for (int i = 0; i < sizeF; i++)
    {
        ...
        {
            //fprintf(stderr, "Chybi radek count=NUMBER");
            return 0;
        }
    }
    fgets(line, size, input);
    //dostal jsem pocet radku
