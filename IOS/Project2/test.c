...
int main() {
    ...
    for (int i = 0; i < NUM_BARBERS; i++) {
        ...
        if (pid == 0) {
            while (1) {
                ...
                sem_wait(&shop->mutex);
                int cut_choice = *((int *) mmap(NULL, sizeof(int), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0));
                shop->num_waiting--;
    ...
    for (int i = 0; i < NUM_CUSTOMERS; i++) {
        ...
        if (customer_pid == 0) {
            ...
            int cut_choice = (rand() % 3) + 1; // Randomly choose a haircut option between 1 and 3
