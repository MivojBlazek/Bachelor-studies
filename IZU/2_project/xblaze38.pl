...
processingList([X, Y | T]):- actualPair(X, Y), processingList([Y | T]). % vezme ze seznamu 1 par a rekurivne se zavola na zbytek seznamu

actualPair(X, Y):- number(X), number(Y), 0 is X mod 2, 1 is Y mod 2. % kontrola cisel a samotne deleni 2 pro zjisteni sudy/lichy
actualPair(X, Y):- number(X), number(Y), 0 is Y mod 2, 1 is X mod 2. % pripadne lichy/sudy
