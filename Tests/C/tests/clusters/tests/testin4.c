...
int load_clusters(char *filename, struct cluster_t **arr)
{
	...
	/*	fprintf(stderr, "%d\n", o_arr[i]->id);*/
		...
		c_arr[i].capacity = 1;
		c_arr[i].obj = &o_arr[i];
		fprintf(stderr, "- TEST: c_arr[i]: %p,Obj: %p, o_arr: %p\n", &c_arr[i], c_arr[i].obj, &o_arr[i]);
	/*	c_arr[i].obj = o_arr[i];*/
	/*	fprintf(stderr, "1\n");
...
fprintf(stderr, "- TEST: c_arr: %p\n", c_arr);	
	*arr = c_arr;
fprintf(stderr, "- TEST: c_arr[0]: %p\n- TEST: ", &c_arr[0]);	
	print_cluster(&c_arr[0]);
fprintf(stderr, "- TEST: ");
	print_cluster(&c_arr[1]);
fprintf(stderr, "- TEST: End of load func\n\n");
	fclose(input);
	return line_cnt;
}
...
**/
	...
	int y = load_clusters(file, &cluster);
	printf("Clusters:\n");
	for (int i = 0; i < y; i++)
	{
		/*TESTING*///printf("Test *cluster -> add: %p\n", &cluster[i]);
		printf("cluster %d: ", i);
