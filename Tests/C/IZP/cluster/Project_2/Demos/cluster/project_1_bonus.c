...
void merge_clusters(struct cluster_t *c1, struct cluster_t *c2)
{
    ...
    // TODO
    //
    //kontrola dostatecne kapacity pro spojeni clusteru
    while (c2->size > c1->capacity - c1->size)
    {
        if (resize_cluster(c1, c1->capacity + CLUSTER_CHUNK) == NULL)
        {
            fprintf(stderr, "ERROR with realloc()\n");
            return;
        }
    }
    //jakmile je dostatecna kapacita, spoji clustery
...
*/
float cluster_distance(struct cluster_t *c1, struct cluster_t *c2, int h_mode)
{
    assert(c1 != NULL);
    assert(c1->size > 0);
    assert(c2 != NULL);
    ...
    for (int i = 0; i < c1->size; i++)
    {
        ...
        {
            //porovnavam kazdy s kazdym a hledam podle parametru h_mode nejkratsi nebo nejvetsi vzdalenost
            if (h_mode == 0 && (obj_distance(&c1->obj[i], &c2->obj[j]) < distance))
                distance = obj_distance(&c1->obj[i], &c2->obj[j]);
            else if (h_mode == 1 && (obj_distance(&c1->obj[i], &c2->obj[j]) > distance))
                distance = obj_distance(&c1->obj[i], &c2->obj[j]);
        }
    }
    return distance;
...
/*
 ...
 adresu 'c1' resp. 'c2'.
*/
void find_neighbours(struct cluster_t *carr, int narr, int *c1, int *c2, int h_mode)
{
    assert(narr > 0);
    ...
    *c2 = 1;
    ...
    float lowest_distance = cluster_distance(&carr[*c1], &carr[*c2], h_mode);
    for (int i = 0; i < narr; i++)
    {
        for (int j = i + 1; j < narr; j++)
        {
            //porovnavam kazdy s kazdym a hledam co nejkratsi
            if (cluster_distance(&carr[i], &carr[j], h_mode) < lowest_distance)
            {
                lowest_distance = cluster_distance(&carr[i], &carr[j], h_mode);
                *c1 = i;
                *c2 = j;
            }
        }
...
int load_clusters(char *filename, struct cluster_t **arr)
{
    ...
    for (int i = 0; i < line_cnt; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            ...
            else if (j != 0 && (atol(token) > 1000 || atol(token) < 0))
            {
                file_error = 1;
            }
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2)
    {
        ...
        * Use parameter [-k] for complete linkage method
        */
        //kontrola parametru
        //kontrola parametru -k na 4. pozici
        if (argc == 4 && argv[3][0] == '-' && argv[3][1] == 'k' && argv[3][2] == '\0')
        {
            h_mode = 1;
            final_narr = atol(argv[2]);
        }
        else if (argc == 3)
        {
            //kontrola parametru -k na 3. pozici
        ...
        {
            fprintf(stderr, "Incompatible arguments!\nUsage: %s 'filename' [number_of_clusters] [-k]\n", argv[0]);
            return 1;
        }
    }
    ...
    if (final_narr > narr || final_narr <= 0)
    {
        ...
        //uvolneni clusteru z duvodu erroru
        while (narr > 0)
        {
            narr = remove_cluster(clusters, narr, narr - 1);
        }
        free(clusters);
        return 1;
    }
    ...
    {
        find_neighbours(clusters, narr, &c1, &c2, h_mode);
