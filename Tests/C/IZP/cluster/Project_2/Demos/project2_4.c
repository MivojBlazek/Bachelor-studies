...
int load_clusters(char *filename, struct cluster_t **arr)
{
    ...
    char line[30]; //mozna muzeme udelat dynamicky alokovane //WTF^2
    int file_error = 0;
    ...
    for (int i = 0; (c = fgetc(input)) != EOF; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            ...
            if (j == 0)
            {
                if (atoi(token) > INT_MAX || atoi(token) < 0)
                {
                    file_error = 1;
                }
                for (int k = 1; i - k >= 0; k++)
                {
                    if (array[i - k][0] == atoi(token))
                    {
                        file_error = 1;
                    }
                }
            }
            if (j != 0 && (atoi(token) > 1000 || atoi(token) < 0))
            {
                file_error = 1;
            }
...
int main(int argc, char *argv[])
{
    ...
    int final_narr = (argc == 3 ? atoi(argv[2]) : 1); //no fuj
