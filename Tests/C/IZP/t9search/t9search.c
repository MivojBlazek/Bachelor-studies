...
/** UZIVATEL ZADAL PRILIS MOC PARAMETRU NEBO CHYBNY **/
	if (argc > 1 + mode)
	{
		fprintf(stderr, "Too many or incorrect parameters.\nUsage: %s [-s] NUMBER_OR_'+' <file.txt\n", argv[0]);
		return 1;
	} 
...
/** CTENI ARGUMENTU DO POLE read_str **/
	...
	if (is_error == 1)
	{
		fprintf(stderr, "There is a parameter longer than 100 characters.\n");
		return 1;	
	}
	else if (is_error == 2)
	{
...
/** CTENI SOUBORU, POROVNAVANI SE SUBSTRINGEM A VYPIS **/
	...
	/** VYTVORENI POLI PRO ULOZENI HODNOT ZE SOUBORU **/
		...
		char numbered_names[MAX_ARR_SIZE];
		char numbered_phones[MAX_ARR_SIZE];
	...
	/** PREPIS arr_of_names NA CISLA A ULOZENI DO names_to_numbers A PRO CISLA TAKY **/
		for (int j = 0; arr_of_names[j] != '\0'; j++)
		{
			numbered_names[j] = chars_to_numbers(arr_of_names[j]);
			numbered_names[j + 1] = '\0';
		}
		for (int j = 0; arr_of_phones[j] != '\0'; j++)
		{	
			numbered_phones[j] = chars_to_numbers(arr_of_phones[j]);
			numbered_phones[j + 1] = '\0';
		}


	/** POROVNAVANI RETEZCU A POCITANI SUBSTRINGU **/
		int count = substring(numbered_names, read_str, size_of_str, &count_of_found);
		int count2 = substring(numbered_phones, read_str, size_of_str, &count_of_found);
